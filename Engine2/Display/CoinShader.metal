...
float4 fog3(float4 position, float4 color) {
    // 1
...
vertex VertexOut coin_vertex_main (VertexIn vIn [[stage_in]],
                                   constant Uniforms &uniforms [[buffer(1)]])
{
    VertexOut vOut;
    vOut.position = uniforms.projectionMatrix * uniforms.viewMatrix * uniforms.modelMatrix * vIn.position;
    vOut.color = float4(vIn.position.y);
    return vOut;
}

fragment float4 coin_fragment_main (VertexOut vIn [[stage_in]])
{
    return fog3(vIn.position, float4(1,1,0,1)) ;
}
