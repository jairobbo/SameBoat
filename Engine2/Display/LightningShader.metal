...
float4 fog5(float4 position, float4 color) {
    // 1
...
vertex VertexOut lightning_vertex_main (VertexIn vIn [[stage_in]],
...
fragment float4 lightning_fragment_main (VertexOut vIn [[stage_in]])
{
    return float4(1);
}
